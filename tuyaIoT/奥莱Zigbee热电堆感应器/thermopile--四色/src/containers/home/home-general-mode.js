...
class GeneralMode extends Component {
  ...
  componentWillReceiveProps(props) {
    ...
    let delay2 = props.delay2;
    let delay3 = props.delay3;
    let power = props.power;
    let switchPir = props.switchPir;
    this.setState({
      d1: Math.ceil(delay/5)*5,
      d2: Math.ceil(delay2/5)*5,
      d3: Math.ceil(delay3/5)*5,
      power: power,
      switchPir: switchPir
    })
  }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.main}>
          {/* <TYText style={{color:'#fff',fontSize:18}}>{'props接收到的开关值：'+this.props.power + " " +typeof this.props.power}</TYText>
            ...
            {/* <TYText>{"处理后的delay:"+this.state.d1}</TYText>
            <TYText>{"处理后的delay:"+this.state.d2}</TYText>
            <TYText>{"处理后的delay:"+this.state.d3}</TYText> */}
          <MyPicker delayName={Strings.getDpLang(delayCode1)} delay={this.state.d1} code={delayCode1} />
          <MyPicker delayName={Strings.getDpLang(delayCode2)} delay={this.state.d2} code={delayCode2} />
          <MyPicker delayName={Strings.getDpLang(delayCode3)} delay={this.state.d3} code={delayCode3} />
        </View>
