...
import TestMode from './home-test-mode';

const { width, convertX: cx, convertY: cy } = Utils.RatioUtils;
const TYDevice = TYSdk.device;
const { mode: modeCode, debugSwitch: debugSwitchCode } = dpCodes;
...
class HomeTopView extends Component {
  ...
  componentWillReceiveProps(props) {
    this.setState({ currMode: props.mode });
  }

  render() {
    //let mode = this.props.mode;
    const debugSwitch = this.props.debugSwitch;
    ...
    const testMode = Strings.getLang('testMode');

    return (
      <View style={[styles.container, { backgroundColor: 'transparent' }]}>
        {true ? (
          <Tab
            //defaultActiveKey={this.state.currMode}
            activeKey={this.state.currMode}
          ...
          >
            ...
            <Tab.TabPane key={'test_mode'} tab={testMode} style={{ flex: 1 }}>
              <TestMode />
            </Tab.TabPane>
          </Tab>
        ) : (
